...
def test_generate(
    ...
    *,
    ...
    input_ids = torch.tensor(tokenizer.encode(prompt, bos=True, eos=False), dtype=torch.long).view(1, -1).repeat(batch_size, 1).to(device_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", type=str, required=True, help="DCP checkpoint path to load (required)")
    ...
    parser.add_argument("--deterministic", action="store_true", help="Use deterministic algorithms wherever possible, may be slower")
